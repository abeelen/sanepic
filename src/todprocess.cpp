...
double **dmatrix(int nrl, int nrh, int ncl, int nch) // c des long normalement
// allocate a double matrix with subscript range m[nrl..nrh][ncl..nch]
{
	...
	//if (!m) nrerror("allocation failure 1 in matrix()"); // nrutil.h a inclure proprement de la lib C NR
	m += NR_END;
	m -= nrl;
