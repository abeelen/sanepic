		...
		long long npixsrc, long iframe, double *S, int para_bolo_indice, int para_bolo_size)
{
	...
	for (long idet=para_bolo_indice*ndet/para_bolo_size;idet<(para_bolo_indice+1)*ndet/para_bolo_size;idet++){
		...
		if(readDataFromDirfile(samples_struct.dirfile_pointers[iframe], dirfile_filename, field, data, ns))
			return 1;
		if(readFlagFromDirfile(samples_struct.dirfile_pointers[iframe], dirfile_filename, field, flag, ns))
			return 1;
		...
		//// Read pointing
		readSampToPix(samples_struct.dirfile_pointers[iframe], dirfile_filename,field, samptopix, ns);
...

{
	...
	for (long idet=para_bolo_indice*ndet/para_bolo_size;idet<(para_bolo_indice+1)*ndet/para_bolo_size;idet++){
		...
		readSampToPix(samples_struct.dirfile_pointers[iframe],  fname,field, samptopix, ns);
