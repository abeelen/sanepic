...
int main(int argc, char *argv[]) {
	...
	for(int ii=0;ii<samples_struct.ntotscan;ii++){ /* for each input fits file */
		...
		if(rank==do_it){ /* if this rank has to do the job ... */
			...
			format[ii]=test_format(samples_struct.fitsvect[ii]); // format = 1 => HIPE, else Sanepic
			...
			return_value+=check_positionHDU(samples_struct.fitsvect[ii],samples_struct.nsamples[ii],ndet_fits, format[ii],check_struct.Check_it); // check presence of reference positions and offsets HDUs
...
#endif
				nb_Nan[ii] += check_NAN_commonHDU(samples_struct.fitsvect[ii],samples_struct.nsamples[ii],bolo_fits, ndet_fits,check_struct.Check_it);
				nb_Nan[ii] += check_NAN_positionHDU(samples_struct.fitsvect[ii],samples_struct.nsamples[ii],bolo_fits, ndet_fits,check_struct.Check_it);
			}
...
#endif
				nb_Nan[ii] += check_NAN_altpositionHDU(samples_struct.fitsvect[ii],samples_struct.nsamples[ii],bolo_fits, ndet_fits,check_struct.Check_it);
			}
	...
	if(rank==0){
		...
		for(int ii=0;ii<samples_struct.ntotscan;ii++){
			...
			cout << FitsBasename(samples_struct.fitsvect[ii]) << " : " << endl << endl;
