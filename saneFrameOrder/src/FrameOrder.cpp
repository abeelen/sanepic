...
uint32_t writeParallelScheme(string outdir, vector<long> order,
...
// Write the Parallelization Scheme for further use.
	...
	for (long ii = 0; ii < samples_struct.ntotscan; ii++) {
		file << Basename(samples_struct.fitsvect[ii]) << " " << order[ii]
				<< endl;
	}
...
void distributeFrames(vector<long> nsamples, vector<long> int_weight,
		vector<float> float_weight, vector<long> & order) {
	/**
	...
	for (long iFrame = 0; iFrame < min(nNode, nFrames); iFrame++) {
		...
		pseudoSample_perProc[index_float[iFrame]] = ceil(
				nsamples[index[iFrame]] * 1.0 / int_weight[index_float[iFrame]])
				* 1.0 / float_weight[index_float[iFrame]];
	}
	...
	for (long iFrame = nNode; iFrame < nFrames; iFrame++) {
		// ... finding the index of the minimum value in pseudoSample_perProc
		it = find(pseudoSample_perProc.begin(), pseudoSample_perProc.end(),
				*(min_element(pseudoSample_perProc.begin(),
						pseudoSample_perProc.end())));
		idCPU = distance(pseudoSample_perProc.begin(), it);
		// ... and assign it to it...
		order[index[iFrame]] = idCPU;
		pseudoSample_perProc[idCPU] += ceil(
				nsamples[index[iFrame]] * 1.0 / int_weight[idCPU]) * 1.0
				/ float_weight[idCPU];
	}

}

void AssignNodeFloatWeight(vector<string> nodeName,
		map<string, float> map_nodeWeight, vector<float> & nodeWeight) {
	/**
	 * Take nodeName corresponding weight in map_nodeWeight associative array, store into float_weight
	 *
	 */
	...
	for (map<string, float>::iterator ii = map_nodeWeight.begin();
			ii != map_nodeWeight.end(); ++ii) {
		string ii_nodeName = (*ii).first;
		float ii_weight = (*ii).second;

		for (vector<string>::iterator jj = nodeName.begin();
				jj != nodeName.end(); ++jj) {
			if (*jj == ii_nodeName) {
...
uint32_t readNodeWeight(string & output, string pathIn,
		map<string, float> & nodeWeight) {
	/**
