...
int write_fits_wcs(string fname, struct wcsprm * wcs, long NAXIS1, long NAXIS2,  char dtype, void *data, string table_name ,bool fits_already_exist, std::vector<string> key, std::vector<int> datatype, std::vector<string> val, std::vector<string> com)
{
	...
	for(int ii=start;ii<2;ii++){
		...
		string sanepic_comment = "This fits file was generated by SANEPIC version " + (string)sanepic_version;
		// add made with Sanepic comment
		if(fits_write_comment(fp, (char *)sanepic_comment.c_str(),  &fits_status))
			fits_status=0; // in case there were a problem, reset error flag and continue...

		// spaces between 'please' and 'check' for a nicer presentation of the message in the FITS header
		sanepic_comment = "For more informations about SANEPIC and for SANEPIC updates, please     check our dedicated website : www.ias.u-psud.fr/sanepic ";
