...
int write_fits_history2(std::string fname, long NAXIS1, long NAXIS2,
		struct param_common dir, struct param_sanePre proc_param,
		struct param_sanePos pos_param, std::vector<double> fcut,
	...
	key_vect.push_back("remove_linear");
	...
	comm_vect.push_back(
			"every scans have the same detector list which name is filled in this field");
	...
	comm_vect.push_back(
			"gaps are projected to a pixel in the map, if so gap filling of noise only is performed iteratively");
	comm_vect.push_back("Sources files format");
	comm_vect.push_back(
			"name of the fits file that was used to mask radiant sources");
	comm_vect.push_back("RA nominal");
	comm_vect.push_back("DEC nominal");
	comm_vect.push_back("Projection type");
	comm_vect.push_back("Axis Type");
	...
	comm_vect.push_back(
			"file containing frequency under which noise power spectra are thresholded");
	comm_vect.push_back("polynomia order");
	comm_vect.push_back(
			"Remove a polynomia fitted to the data to reduce fluctuations on timescales larger than the length of the considered segment ?");
	comm_vect.push_back("Do we remove a baseline from the data ?");
	comm_vect.push_back(
			"Correlations between detectors are not included in the analysis ?");
	comm_vect.push_back(
			"Do we fill the gaps in the timeline with White noise + baseline ?");

	// [saneInv]
	comm_vect.push_back(
			"this file contains the matrix you want to invert (same for all scans)");
	comm_vect.push_back("covariance matrices path");
	comm_vect.push_back(
			"Each scan has his own covmat named basename(scan_filename) + cov_suffix");

	// [sanePS]
	comm_vect.push_back("ell file (the same for all the scans)");
	comm_vect.push_back(
			"each scan has an ell file named : basename(scan_filename) + ell_suffix");
	comm_vect.push_back("MixingMatrix file is the same for all the scans");
	comm_vect.push_back(
			"each scan has a mixmat file named : basename(scan_filename) + MixingMatrix_suffix");
	comm_vect.push_back("number of noise component estimated in sanePS");
	comm_vect.push_back(
			"A map whose signal will be subtracted from the data for the noise estimation step");
	comm_vect.push_back("Do we save the session after each step ?");

	// [sanePic]
	comm_vect.push_back(
			"iterW = -1 -> no save on disk will be done. iterW > 0 -> save on disk every iterW iterations");
	...
	value_vect.push_back(StringOf(pos_param.lon));
	value_vect.push_back(StringOf(pos_param.lat));
	value_vect.push_back(pos_param.projcode);
	value_vect.push_back(pos_param.axistype);
	...
	value_vect.push_back(
			StringOf(proc_param.remove_polynomia ? "True" : "False"));
	value_vect.push_back(StringOf(proc_param.remove_linear ? "True" : "False"));
	value_vect.push_back(StringOf(proc_param.CORRon ? "True" : "False"));
	value_vect.push_back(StringOf(proc_param.fill_gap ? "True" : "False"));
	...
	char *tform[] = { tableFormat(key_vect), tableFormat(value_vect),
			tableFormat(comm_vect) };
	...
	if (fits_create_tbl(fptr, BINARY_TBL, key_vect.size(), 3, ttype, tform,
			tunit, (char*) "History", &fits_status))
		return 1;
	if (fits_write_col(fptr, TSTRING, 1, 1, 1, key_vect.size(), data,
			&fits_status))
		return 1;
	if (fits_write_col(fptr, TSTRING, 2, 1, 1, value_vect.size(), data_value,
			&fits_status))
		return 1;
	if (fits_write_col(fptr, TSTRING, 3, 1, 1, comm_vect.size(), data2,
			&fits_status))
		return 1;
...
int write_fits_META(string fname, long NAXIS1, long NAXIS2, string path,
		struct param_sanePre proc_param, struct param_sanePos pos_param,
		std::vector<double> fcut, struct samples samples_struct, long ncomp) {
	...
	if (fits_write_key(fp, TSTRING, (char*) keyname.c_str(),
			(char*) value.c_str(), (char*) comm.c_str(), &fits_status)) {
		fits_report_error(stderr, fits_status);
		return 1;
	}
