...
// read and check that the saved Parallelization Scheme correspond to the actual data
{
	...
	/* 	long *dummy_ruleorder, *dummy_frnum, *dummy_nsamples, * dummy_ntotscan, *dummy_size;
	 *
	 * 	read_ParallelizationScheme(testfile2, &dummy_ruleorder, &dummy_frnum, &dummy_nsamples, &dummy_ntotscan &dummy_size);
	 *
	 * TODO: Check for compatible arguments,
	 *       ie same size, ntotscan and nsamples.
	 * if good :
