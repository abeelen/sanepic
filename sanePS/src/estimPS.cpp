...
int EstimPowerSpectra(struct detectors det, struct param_sanePre proc_param, struct param_common dir, struct param_sanePos pos_param, struct param_sanePS structPS, struct samples samples_struct,
...
//	printf("\nEstimation procedure started : \n");
	...
	vect = dmatrix(0,structPS.ncomp-1,0,det.ndet-1);
...
#endif
	if(expectation_maximization_algorithm(structPS.fcutPS, nbins, det.ndet, structPS.ncomp, samples_struct.nsamples[iframe], proc_param.fsamp,
...
#endif
	if(write_to_disk(dir.output_dir, samples_struct.fits_table[iframe], det, nbins, ell, mixmat, Rellth,
	...
	free_dmatrix(mixmat,0,det.ndet-1,0,structPS.ncomp-1);

	free_dmatrix(commonm2,0,structPS.ncomp,0,samples_struct.nsamples[iframe]-1);
	free_dmatrix(vect,0,structPS.ncomp-1,0,det.ndet-1);
	delete [] ell;
